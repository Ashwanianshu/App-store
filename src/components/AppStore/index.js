...
class AppStore extends Component {
  ...
  filteredProject = () => {
    ...
    const filterList = tabsList.filter(i => i.tabId === id)
    const value = filterList[0].tabId
    const newestList = appsList.filter(i => i.category === value)
    return newestList
  }

  render() {
    const {searchInput, id} = this.state
    const newFilteredList = this.filteredProject()
    const filteredList = newFilteredList.filter(i =>
